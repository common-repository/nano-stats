...
Plugin URI: http://wordpress.org/plugins/nano-stats
...
function nanostats_install() {
	...
	add_option('nanostats_region','europe-the-netherlands','','yes');
...
if (is_admin()) {
	...
	function nanostats_admin_menu() {
		add_options_page('NaNo Stats Options', 'NaNo Stats', 'administrator','nanostats', 'nanostats_option_page');
	}
...
add_action('init', 'nanostats_enqueue');
...
function showNaNoWidget_participant($atts) {
	...
	$extra = '';
	if (isset($atts['showpercent']) && $atts['showpercent']=='true')
		$extra .= ',pc';
	if (isset($atts['showdays']) && $atts['showdays']=='true')
		$extra .= ',days';
	if (isset($atts['showwords']) && $atts['showwords']!='true')
		$extra .= ',wc';
	if (isset($atts['goal']))
		$extra .= ',goal='.$atts['goal'];
	if (isset($atts['show_username']) && $atts['show_username']=='true') {
